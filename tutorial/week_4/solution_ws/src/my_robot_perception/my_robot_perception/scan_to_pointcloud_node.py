...
adding a Node action to their own launch file or running it directly:
    ros2 run my_robot_perception scan_to_pointcloud
"""
import math
import struct
...
# Importing tf2_geometry_msgs registers the do_transform_point() handler for
# PointStamped (used below) as a side effect of the import.
...
class ScanToPointCloudNode(Node):
    """Transforms scan beams into a base_link PointCloud2 on scan_points."""
    ...
    def _scan_cb(self, msg: LaserScan) -> None:
        ...
        for x, y, z in pts_lidar:
            ...
            ps.point.y = y
            ps.point.z = z
            transformed = tf2_geometry_msgs.do_transform_point(ps, transform)
            out_points.append((
