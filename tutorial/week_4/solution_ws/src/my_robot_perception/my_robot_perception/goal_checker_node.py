"""Goal checker: watches odom and latches a one-shot 'goal reached' signal.

Publishes a latched Bool on goal_reached, a latched PointStamped goal
coordinate on goal_point (single source of truth for nav nodes), and a
latched RViz Marker on goal_marker. Goal pose and threshold are parameters.
"""
import math
...
class GoalCheckerNode(Node):
    """Latches goal_reached when the robot first enters the goal radius.

    Owns the latched goal_point / goal_marker so nav nodes and RViz read one
    authoritative goal rather than duplicating the parameter.
    """
