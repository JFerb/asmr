...
def generate_launch_description():
    ...
    world_path = PathJoinSubstitution([bringup_share, 'worlds', 'obstacle_world.sdf'])
    ...
    ld.add_action(
        Node(
            package='ros_gz_sim',
            executable='create',
            arguments=['-topic', 'robot_description', '-name', 'my_robot', '-x', '1.0', '-y', '3.0', '-z', '0.1']
        )
    )
    ...
    ld.add_action(
        Node(
            ...
            parameters=[{'goal_x': 9.0, 'goal_y': 0.0}]
        )
    )
